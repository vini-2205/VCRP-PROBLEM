...
iteracoes_list = [500,1000,2500,5000,10000]
...
def solucao_uma_formiga(vertices, arestas, capacidade_limite, demanda, feromonios, num_caminhoes, formiga, iteracao):
    ...
    while vertices and caminhões_usados < num_caminhoes:
        rota = []
        capacidade_restante = capacidade_limite
        while vertices:
            probabilidades = [
            ...
            if soma_probabilidades == 0:
                probabilidades = np.ones(len(vertices))
            else:
                probabilidades /= soma_probabilidades
            
            # Verifique se probabilidades contém NaN ou inf e trate conforme necessário
            if np.any(np.isnan(probabilidades)) or np.any(np.isinf(probabilidades)):
                probabilidades = np.ones(len(vertices))
                soma_probabilidades = len(vertices)
            else:
                probabilidades = np.clip(probabilidades, 0, 1)  # Garantir que todas as probabilidades sejam válidas
                soma_probabilidades = np.sum(probabilidades)

            probabilidades /= soma_probabilidades
            
            proxima_cidade = np.random.choice(vertices, p=probabilidades)

            if capacidade_restante - demanda[proxima_cidade] >= 0:
                capacidade_restante -= demanda[proxima_cidade]
                rota.append(proxima_cidade)
                vertices.remove(proxima_cidade)
                cidade_atual = proxima_cidade
            else:
                break
        
        solucao.append(rota)
        cidade_atual = 1  # Volta ao depósito
        caminhões_usados += 1
...
def rodar_aco_para_variadas_iteracoes():
    coordenadas, demanda, capacidade_limite = ler_arquivo_vrp('A-4.vrp')
